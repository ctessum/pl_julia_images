...
mv $JOB_DIR/data/data.json $FILENAMES_DIR
mv $MERGE_DIR/*.jl $FILENAMES_DIR
mv $MERGE_DIR/*.bson $FILENAMES_DIR
mv $MERGE_DIR/*.csv $FILENAMES_DIR
...
su -c "julia --project=/home/julia/project/ --color=no --heap-size-hint=800M $MERGE_DIR/filenames/test.jl" julia
